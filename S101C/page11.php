...
<div class='left_content'>
			...
			<img src="images/page/24.jpg" width="850" border="0" usemap="#Map" title="(相反數有時指一對數，有時也只指一個數。例如 +7 和 –7 也是一對相反數，另一方面，我們也會描述 +7 的相反數是 –7 或 – 7 的相反數是 + 7 。)+5 和 –5 兩個數所含的數字相同，但符號不同，這不計符號所含的數字則稱為絕對值， 絕對值就是不理會正負號的數值，只描述該數的大小。例如 +5 和 –5 的絕對值都是5， + 9和 –9 的絕對值都是9。［註：絕對值非課程所需，但認識絕對值這概念日後有助更易了解或描述一些其他概念。］數學家將所有自然數，以及它們的相反數，再加上 0 合稱為整數 (自然數也稱為正整數)。正數及負數也合稱有向數，因除了絕對值以外，也提供一個方向的意義 (0 非正非負，不屬於有向數)。數學家為協助了解及日後的運算，也創製了「數線」。數線性質如下："/>
			<map name="Map" id="Map">
				<area shape="rect" coords="62.309820193638,319.77869986169,78.769017980636,336.23789764869" href="./page11_01.php">
			</map>
			</div>
		</div><!--left_content-->
